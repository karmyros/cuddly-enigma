...
<b>Функція count</b> використовує динамічне програмування для ефективного підрахунку кількості послідовностей, які відповідають <i>умові відсутності трьох одиниць, що йдуть послідовно.</i> 

Вона створює двовимірний масив dp, де dp[i][j] представляє кількість послідовностей довжини i, що закінчуються j послідовними одиницями (0, 1 або 2).
...
<h2><b>Практичне завдання №6</b></h2>

У <b>чотири змінні координат(x1, y1; x2, y2)</b> вносяться дані координат вектора.

У <b>змінних різниці dx, dy</b> від кінцевих координат віднімаються початкові. Тоді їхні значення підносяться до квадрату і додаються.

Тоді знаходиться корінь суми квадратів різниць, який і виводиться у результаті роботи програми.

<hr>

<h2><b>Практичне завдання №7</b></h2>

Вводяться змінні для зберігання:

<ul><li>координат центрів кіл - x1, y1, x2, y2,</li>
<li>радіусів двох кіл - r1 і r2.</li></ul>

Значення змінних вводяться користувачем. Тоді, подібно до завдання №6, рахується різниця координат і зберігається у змінних dy і dx. Змінна відстані між центрами кіл також обчислюється ідентично до 6 завдання.

Тоді проводиться перевірка отриманих змінних.

<ol><li>Перевіряємо, чи накладаються кола. У такому випадку відстань між центрами нульова, а радіуси кіл однакові. Якщо ці умови справджуються, змінна-лічильник count дорівнює -1, <i>а кількість точок перетину є нескінченною.</i></li>
<li>Тоді перевіряємо, чи кола мають спільну точку взагалі. Якщо відстань між центрами більша за суму радіусів, або якщо вона менша за модуль їхньої різниці, тоді <i>кола не мають жодної спільної точки.</i> Змінна count не змінює значення.</li>
<li>Якщо попередні умови не справджуються, два кола мають хоча б одну спільну точку, але не нескінченну кількість. Якщо відстань між колами дорівнює сумі радіусів або вона дорівнює модулю їхньої різниці, <i>кола дотикаються одне до одного рівно в одній точці.</i></li>
<li>У іншому випадку ми маємо дві точки перетину кіл.</li>
</ol>

Після завершення усіх перевірок кількість точок перетину виводиться у консоль, а програма завершується.

<hr>

<h2><b>Практичне завдання №8</b></h2>

<b>Функція findFact обчислює факторіал числа.</b>

Після введення слова користувачем програма перевіряє його довжину. Якщо слово довше 14 символів, виводиться повідомлення про помилку, і цикл починається знову.

Якщо довжина слова прийнятна, програма переходить до обчислення кількості анаграм. <i>Для цього використовується формула, яка враховує загальну кількість перестановок (факторіал довжини слова) і ділить її на добуток факторіалів повторень кожної літери. </i>
Факторіал рахується завдяки функції findFact().

Програма створює масив charCount для підрахунку кількості кожного символу в слові за їхнім кодом ASCII. Потім обчислюється добуток факторіалів повторень символів.
У кінці циклу програма ділить загальний факторіал на отриманий добуток факторіалів повторень і виводить результат - кількість можливих анаграм для введеного слова.

<hr>

<h2><b>Практичне завдання №2</b></h2>

Як у попередніх завданнях, у користувача є можливість повторювати роботу програми нескінченно з будь-якими значеннями, що задовольняють вимоги.

Два цілих значення, одне з яких <b>(x)</b> менше за інше або дорівнює йому<b>(y)</b>, записуються у відповідні змінні. 

Отримані змінні перевіряються на відповідність умові <i>0 <= x <= y < 2^31</i>. За умови невідповідності програма перезапускається, сповістивши користувача.

Спосіб отримання вихідного результату залежить від значення різниці кінцевої і початкової точки:
<ul><li>за умови, <i>якщо вона невелика (до 5 включно)</i>, кроки будуть обчислені через функцію stepsLoop(),</li>
<li>а <i>якщо вона буде більшою за 5</i>i>, використовуватиметься функція stepsMath().</li></ul>

<b>Функція stepsLoop()</b> використовує циклічний алгоритм для обчислення кількості кроків. Вона поступово збільшує довжину кроку, перевіряючи, чи можна досягти кінцевої точки, і коригує розмір кроку за необхідності. Після закінчення роботи вона виводить послідовність кроків і повертає їх загальну кількість.
Змінна steps потрібна для збереження кількості кроків і збільшується кожного повтору циклу while.

Сам цикл while здійснюється, поки <b>змінна path</b>, що зберігає суму координат початкової точки х і вже пройденого шляху, не досягне значення кінцевої точки y. 

Пройдений шлях формується з розмірів поточного кроку <b>currStep</b>, який може бути меншим, більшим за попередній або дорівнювати йому. 

Те, як потрібно змінити значення currStep, і чи варто його змінювати, визначається через застосування змінної <b>checkSteps</b>. Нею є сума чисел від 1 до розміру поточного кроку, створена через цикл for. 

Спершу йде перевірка на те, чи варто зменшити довжину кроку, під час якої застосовується currStep і path.

Якщо вона не спрацьовує, йде перевірка на те, чи варто збільшити цикл. Під час неї застосовуються path, checkSteps і currStep. 

Кожного повтору циклу у консоль виводиться поточна довжина кроку. 

<b>Функція stepsMath</b> використовує математичну формулу для обчислення кількості кроків на основі відстані між двома точками. Змінна distance зберігає цю різницю між y і x для зручнішого сприйняття і читання у формулі. 

Для вибору необхідної функції застосовується тернарний оператор. 

Фінальне значення виводиться на екран і програма перезапускається.
